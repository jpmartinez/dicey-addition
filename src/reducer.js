...
export function reducer(state, action) {
	...
	switch (action.type) {
		...
		case 'setPlayer':
			return merge(state, { players: action.value });
		case 'setTimer':
		...
		case 'calculate': {
			...
			let sum1 =
				parseInt(`${player1Move[0]}${player1Move[1]}`) + parseInt(`${player1Move[2]}${player1Move[3]}`);
			let sum2 =
				parseInt(`${player2Move[0]}${player2Move[1]}`) + parseInt(`${player2Move[2]}${player2Move[3]}`);
			...
			const player1 = {
				...
				score:
					winner === state.players.player1.name
			...
			const player2 = {
				...
				score:
					winner === state.players.player2.name
